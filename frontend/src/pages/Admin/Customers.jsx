  ...
  const completeAction = async () => {
    ...
    try {
      ...
      if (action.type === 'delete')
        await api.delete(`/admin/users/${action.customer._id}`);
      else
        await api.patch(`/admin/users/${action.customer._id}/${action.type}`);
      setAction(null);
      loadCustomers();
    } catch (requestError) {
      setError(
  ...
  const actionCopy =
    action?.type === 'delete'
      ? {
          ...
          message:
            'This soft-deletes the account and removes it from customer management.',
      ...
      : action?.type === 'suspend'
        ? {
            ...
            message:
              'The customer will no longer be able to sign in until an administrator reactivates the account.',
            confirm: 'Suspend',
            danger: true,
          }
        : {
            title: 'Reactivate customer?',
            message: 'The customer will be able to sign in again.',
            confirm: 'Reactivate',
            danger: false,
          };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <section style={{ ...surface, overflow: 'hidden' }}>
          ...
          {loading ? (
                    ...
                    <tr key={customer._id}>
                      {
                        <>
                          ...
                          >
                            <div style={{ color: T.slate, fontSize: 11 }}>
                              {customer.email}
                            </div>
                            <div
                              style={{
                          ...
                          >
                            <StatusBadge
                              value={customer.isActive ? 'active' : 'suspended'}
                            />
                          </td>
                          <td
                            style={{
                            ...
                            <div style={{ display: 'flex', gap: 10 }}>
                              {customer.isActive ? (
                                <ActionButton
                                  tone="warning"
                                  onClick={() =>
                                    setAction({ type: 'suspend', customer })
                                  }
                                >
                                  <Ban
                                    size={13}
                                ...
                                <ActionButton
                                  tone="success"
                                  onClick={() =>
                                    setAction({ type: 'activate', customer })
                                  }
                                >
                                  <UserCheck
                                    size={13}
                              ...
                              <ActionButton
                                tone="danger"
                                onClick={() =>
                                  setAction({ type: 'delete', customer })
                                }
                              >
                                <Trash2
                                  size={13}
            ...
            <EmptyTable label="No customers match the selected filters." />
          )}
      ...
      <ConfirmModal
        ...
        confirmText={actionCopy.confirm}
